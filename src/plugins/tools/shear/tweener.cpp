...
void Tweener::init(TupGraphicsScene *scene)
{
    ...
    k->configurator->initStartCombo(k->framesCount, k->initFrame);
}
...
void Tweener::setSelection()
{
    ...
    /*
    int bottomBoundary = (ZLAYER_BASE) + (k->scene->currentLayerIndex()*ZLAYER_LIMIT);
...
void Tweener::applyTween()
{
    ...
    if (!k->scene->currentScene()->tweenExists(name, TupItemTweener::Shear)) {
        ...
        foreach (QGraphicsItem *item, k->objects) {
                 TupLibraryObject::Type type = TupLibraryObject::Item;
                 TupScene *scene = k->scene->currentScene();
                 TupLayer *layer = scene->layerAt(k->currentTween->getInitLayer());
                 ...
                 if (k->initFrame != k->currentTween->getInitFrame()) {
                     ...
                     request = TupRequestBuilder::createItemRequest(k->initScene,
                                                                   k->initLayer,
...
void Tweener::updateMode(TupToolPlugin::Mode mode)
{
    ...
    if (k->mode == TupToolPlugin::Edit) {
        ...
        if (k->currentTween) {
            ...
            if (k->objects.isEmpty()) {
                k->objects = k->scene->currentScene()->getItemsFromTween(k->currentTween->getTweenName(), TupItemTweener::Shear);
