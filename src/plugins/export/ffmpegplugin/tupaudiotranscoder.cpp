 ...
 */
int TupAudioTranscoder::openInputFile(const char *filename,
 ...
 * @return Error code (0 if successful)
 */
int TupAudioTranscoder::openOutputFile(const char *filename,
                            AVCodecContext *inputCodecContext,
                            ...
                            AVCodecContext **ouputCodecContext)
{
    ...
    error = avcodec_parameters_from_context(audioOutputStream->codecpar, avctx);
    if (error < 0) {
                              ...
                              int *dataPresent, int *finished)
{
    /*
    #ifdef TUP_DEBUG
        qDebug() << "[TupAudioTranscoder::decodeAudioFrame()]";
    #endif
    */
                              ...
                              int *dataPresent)
{
    /*
    #ifdef TUP_DEBUG
        qDebug() << "[TupAudioTranscoder::encodeAudioFrame()]";
    #endif
    */
                                 ...
                                 AVCodecContext *outputCodecContext)
{
    /*
    #ifdef TUP_DEBUG
        qDebug() << "[TupAudioTranscoder::loadEncodeAndWrite()]";
    #endif
    */
...
{
    #ifdef TUP_DEBUG
        qDebug() << "[TupAudioTranscoder::mergeAudioStream()]";
    #endif
    ...
    while (1) {
        ...
        while (av_audio_fifo_size(fifo) < outputFrameSize) {
            ...
            {
                #ifdef TUP_DEBUG
                    qDebug() << "[TupAudioTranscoder::mergeAudioStream()] - Tracing readDecodeConvertAndStore() method...";
                #endif
                goto cleanup;
            }
        ...
        while (av_audio_fifo_size(fifo) >= outputFrameSize ||
            ...
            if (loadEncodeAndWrite(fifo, audioOutputFormatContext,
                                      audioOutputCodecContext)) {
                #ifdef TUP_DEBUG
                    qDebug() << "[TupAudioTranscoder::mergeAudioStream()] - Tracing loadEncodeAndWrite() method...";
                #endif
                goto cleanup;
            }
        ...
        if (finished) {
            ...
            do {
                ...
                if (encodeAudioFrame(nullptr, audioOutputFormatContext,
                    ...
                    #ifdef TUP_DEBUG
                        qDebug() << "[TupAudioTranscoder::mergeAudioStream()] - Audio process done!";
                    #endif
                    goto cleanup;
                }
            } while (data_written);
