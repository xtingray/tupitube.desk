        ...
        if (removeTempVideo && !tempVideoPath.isEmpty()) {
            ...
            if (videoDir.exists()) {
                if (!videoDir.removeRecursively()) {
                    ...
                    #ifdef TUP_DEBUG
                        qDebug() << "[TupLibraryWidget::loadSequenceFromDirectory()] - "
                                    "Temporary (video) folder was removed succesfully! ->"
                                 << tempDir;
                    #endif
                }
            }
...
void TupLibraryWidget::callExternalEditor(QTreeWidgetItem *item, const QString &software)
{
    if (item) {
        ...
        if (object) {
            ...
            #ifdef TUP_DEBUG
                qDebug() << "[TupLibraryWidget::callExternalEditor()] - "
                            "Fatal Error: No object related to the current library item was found! ->" << id;
            #endif
        }
    } else {
        #ifdef TUP_DEBUG
...
void TupLibraryWidget::executeSoftware(const QString &software, QString &path)
{
    ...
    if (path.length() > 0 && QFile::exists(path)) {
        ...
        #ifdef TUP_DEBUG
            qDebug() << "[TupLibraryWidget::executeSoftware()] - "
                        "Fatal Error: Item path either doesn't exist or is empty";
        #endif
    }
}
...
void TupLibraryWidget::updateCurrentSoundPath()
{
    #ifdef TUP_DEBUG
        qDebug() << "[TupLibraryWidget::updateCurrentSoundPath()]";
    #endif

    QString soundID = display->getSoundID();
    currentSound = library->getObject(soundID);
    SoundResource resource = project->getSoundResource(soundID);
    display->updateCurrentSoundPath(resource.path);
}
