...
static void end_atomic_default(MyPaintSurface *surface, MyPaintRectangle *roi)
{
...
void mypaint_tiled_surface_end_atomic(MyPaintTiledSurface *self, MyPaintRectangle *roi)
{
    ...
    // SQA: Check if this code is required
                      ...
                      float r_aa_start)
{
    ...
    //   A = pi * r * r
    //   r = sqrt(1/pi)
    const float rad_area_1 = sqrtf(1.0f / (float) M_PI);
                      ...
                      float hardness, float aspect_ratio, float angle)
{
    ...
    float angle_rad = angle / (float) (360*2*M_PI);
    ...
    if (y0 < 0)
        y0 = 0;
    if (x1 > MYPAINT_TILE_SIZE-1)
        x1 = MYPAINT_TILE_SIZE-1;
    if (y1 > MYPAINT_TILE_SIZE-1)
        y1 = MYPAINT_TILE_SIZE-1;
    ...
    if (radius < 3.0f) {
        ...
        for (yp = y0; yp <= y1; yp++) {
            int xp;
            for (xp = x0; xp <= x1; xp++) {
                const float rr = calculate_rr_antialiased(xp, yp, x, y, aspect_ratio,
                                                          sn, cs, one_over_radius2, r_aa_start);
        ...
        for (yp = y0; yp <= y1; yp++) {
            int xp;
            for (xp = x0; xp <= x1; xp++) {
                const float rr = calculate_rr(xp, yp, x, y, aspect_ratio,
...
// Must be threadsafe
void process_op(uint16_t *rgba_p, uint16_t *mask, int tx, int ty, OperationDataDrawDab *op)
{
    ...
    if (op->colorize) {
        draw_dab_pixels_BlendMode_Color(mask, rgba_p, op->color_r, op->color_g, op->color_b,
...
void update_dirty_bbox(MyPaintTiledSurface *self, OperationDataDrawDab *op)
{
...

{
    ...
    if (op->hardness == 0.0f)
        return FALSE; // infintly small center point, fully transparent outside
    if (op->opaque == 0.0f)
        return FALSE;
    ...
    if (op->aspect_ratio < 1.0f)
        op->aspect_ratio = 1.0f;
...
void get_color(MyPaintSurface *surface, float x, float y, float radius,
